...
Edit `<repo>/.env.local` and replace the `DATABASE_URL` value with the new pooler connection string. Make sure `?pgbouncer=true&connection_limit=1` is preserved at the end if it was there.
...
Once verified, update `<claude-project-memory>/MEMORY.md` if the credential rotation date or Supabase project state changed materially.
